const DATASETS = {
	...
	souvenirs: {
		url: "https://raw.githubusercontent.com/logos-circle-ruse/data/refs/heads/main/website/interactive-ruse/souvenirs.json",
		tagColours: {}
	}
};
...
function buildPopup(place) {
	...
	return DOMPurify.sanitize(`
		...
		${place.image ? `<button type="button" class="popup-preview-link">Preview</button>` : ""}
	`, { ADD_ATTR: ["target"] });
}

function openPreviewModal(place) {
	const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${place.latitude},${place.longitude}`;

	document.getElementById("preview-name").textContent = place.name;
	document.getElementById("preview-description").textContent = place.short_description || "";
	...
	buildPreviewGallery(place);

	document.getElementById("preview-overlay").classList.remove("view-hidden");
	document.body.classList.add("modal-open");
}

function buildPreviewGallery(place) {
	const gallery = document.getElementById("preview-gallery");
	gallery.innerHTML = "";

	const historical = Array.isArray(place.historical) ? place.historical : [];
	if (historical.length === 0) return;

	const image = document.getElementById("preview-image");
	const defaultSrc = place.image || "";

	historical.forEach((src, index) => {
		const thumb = document.createElement("img");
		thumb.src = src;
		thumb.alt = `${place.name} - historical photo ${index + 1}`;
	...
	if (defaultSrc) {
		...
		currentThumb.src = defaultSrc;
		currentThumb.alt = `${place.name} - current photo`;
		...
		gallery.prepend(currentThumb);
	}
}

function closePreviewModal() {
	document.getElementById("preview-overlay").classList.add("view-hidden");
	document.body.classList.remove("modal-open");
}

function wirePreviewModal() {
	document.getElementById("preview-close").addEventListener("click", closePreviewModal);
	document.getElementById("preview-overlay").addEventListener("click", event => {
		if (event.target.id === "preview-overlay") closePreviewModal();
	});
...
function wirePopupPreview(marker, place) {
	marker.on("popupopen", event => {
		const el = event.popup.getElement();
		const button = el.querySelector(".popup-preview-link");
		if (!button) return;

		button.addEventListener("click", () => {
			marker.closePopup();
			openPreviewModal(place);
		});
	});
}
...
async function loadDataset(datasetName, map, layerGroup) {
				...
				${rows.map(place => {
					...
					return `
						<div class="place-card">
							<div class="place-name" style="background:${rowColour}">${place.name}</div>
							<div class="tag-list">${tags}</div>
							${place.image ? `<button type="button" class="place-view-btn" data-place-name="${safeName}">View</button>` : ""}
							${datasetName === "food" || datasetName === "souvenirs" ? `<a href="${mapsUrl}" target="_blank" rel="noopener noreferrer" class="place-directions-btn">How to get there</a>` : ""}
						</div>
					`;
				}).join("")}
			</div>
	...
	listContainer.addEventListener("click", event => {
		const button = event.target.closest(".place-view-btn");
		if (!button) return;
		const place = places.find(p => p.name === button.dataset.placeName);
		if (place) openPreviewModal(place);
	});
...
async function initMap() {
	...
	document.getElementById("map-list").classList.add("view-hidden");
	buildViewToggle(map);
	map.invalidateSize();
}
